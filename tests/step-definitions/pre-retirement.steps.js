...
When('User sets SSN to No', async () => {
  // User scrolls down to SSN Y/N Radio button
  let SSNNoRadioBtn = await HomePage.scrollDownTillSSNRadioBtn();
  expect(SSNNoRadioBtn).to.be.true;
  

  //Select SSN No option radio button 
  let isSSNNOOPtionSelected = await HomePage.selectSSNNoRadioBtn();
  expect(isSSNNOOPtionSelected).to.be.true;
})
Then('User should not see respective SSN fields', async () => {
  // Verify SSN fields are not displayed when we select No radio button
  let [maritalStatusOptions,socailSecurityOverrideAmountTextBox] = await HomePage.validateSSNFieldsNotDisplayed();
  expect(maritalStatusOptions).to.be.false;
  expect(socailSecurityOverrideAmountTextBox).to.be.false;
